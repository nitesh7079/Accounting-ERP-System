...
const PaymentVoucher = () => {
  ...
  const generateVoucherNumber = () => {
    ...
    setFormData(prev => ({ ...prev, voucherNumber: `PAY-${timestamp}` }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const voucherData = {
        ...
        entries: [
          ...formData.entries.map(e => ({ ledger: e.ledger, type: 'Dr', amount: parseFloat(e.amount) })),
          { ledger: formData.account, type: 'Cr', amount: totalAmount }
        ]
      };
      ...
      setError(error.response?.data?.message || 'Error creating payment voucher');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      ...
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        <div className="bg-pink-700 text-white px-4 py-2 mb-4 flex justify-between items-center">
